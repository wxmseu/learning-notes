...
`SHOW GLOBAL STATUS LIKE 'Com_______';`（7个下划线） 或者 `SHOW SESSION STATUS LIKE 'Com_______';`
...
​	long_query_time=2
更改后记得重启MySQL服务，日志文件位置：/var/lib/mysql/主机名-slow.log
...
![image-20221106134011179](C:\Users\xiaom\AppData\Roaming\Typora\typora-user-images\image-20221106134011179.png)
...
![image-20221106141608203](C:\Users\xiaom\AppData\Roaming\Typora\typora-user-images\image-20221106141608203.png)

##### 注意事项

- 多条件联合查询时，MySQL优化器会评估哪个字段的索引效率更高，会选择该索引完成本次查询；可以加入sql提示来指定索引。
...
全局锁就是对整个数据库实例加锁，枷锁后整个实例就处于只读状态，后续的DML的写语句，DDL语句已经更新操作的事务语句都将被阻塞。

其典型的使用场景是全库的逻辑备份，对所有的表进行锁定，从而获取一致性视图，保证数据的完整新。

#### 使用

加全局锁：`flush tables with read lock;`

执行备份操作，此操作需要推出mysql，在命令行执行：`mysqldump -uroot -p1234 itcast>itcast.sql;`
...
1. 如果在主库上备份，那么在备份期间都不饿能执行更新，业务基本山就得停摆；
...
表级锁，每次操作锁住整张表。锁定粒度大，发生锁冲突的概率最高，并发度最低。应用在MyISAM、InnoDB、BDB等引擎中。
...
2. 释放锁：unlock tables 或者将客户端断开连接

![image-20221107162342254](C:\Users\xiaom\AppData\Roaming\Typora\typora-user-images\image-20221107162342254.png)

![image-20221107162659892](C:\Users\xiaom\AppData\Roaming\Typora\typora-user-images\image-20221107162659892.png)

#### 元数据锁

MDL加锁过程是系统自动控制，无需显示使用，在访问一张表的时候会自动加上。MDL锁主要作用是维护表元数据的数据一致性，在表上有活动事务的时候，不可以对元数据进行写入操作。**为了避免DML和DDL冲突，保证读写的正确性。**

在Mysql5.5中引入MDL，当对一张表进行增删改查的时候，加MDL读锁（共享）；当对表结构进行变更操作的，加MDL写锁（排他）。

| 对应sql                                     | 锁类型                                 | 说明                                             |
| ------------------------------------------- | -------------------------------------- | ------------------------------------------------ |
| lock tables xxx read/write                  | SHARED_READ_ONLY/SHARED _NO_READ_WRITE |                                                  |
...
InniDB的数据时基于索引组织的，行锁是通过对索引上的索引项加锁来实现的，而不是对记录加的锁。对于行级锁，主要分为以下三类：

1. 行锁（Record Lock）：锁定单个行记录的锁，防止其他事务对此进行update和delete。在RC和RR隔离级别下都支持。
2. 间隙锁（Gap Lock）：锁定索引记录间隙（不含该记录），确保索引记录间隙不变，防止其他事务在这个间隙中insert，产生幻读。在RR隔离级别下支持。
